...
\begin{Figure}
    ...
    \input{perm_eps.tex}
    \captionof{figure}{Pruning and enrichment. The new weights are depicted above each branch. Upper branch: the weight of the partial polymer $W_n^{(l)} > W_+^{(l)}$, so the polymer is split and the weight of both polymers is halved. The middle branch: $W_n^{(l)} < W_-^{(l)}$ the weight is doubled and the polymer continues growing. Bottom branch: $W_n^{(l)} < W_-^{(l)}$ and the polymer is pruned.} 
    \vspace{18 mm}
    \label{fig:my_label}
\end{Figure}
