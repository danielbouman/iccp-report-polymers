...
\end{gather}
where $W_n$ is the weight of a polymer, and the sum is over the whole phase space. If the phase space is sampled randomly $K$ times, then
\begin{gather}
...
The weight of a single bead $w_j^{(l)}$ is given by
\begin{gather}
    w_j^{(l)} = e^\frac{-E(\theta_j)}{k_BT},
\end{gather} where $l$ denotes the bead that will be added and $E(\theta_j)$ is the energy associated with position $\theta_j$. The total weight of each polymer is the product of the all the bead weights
